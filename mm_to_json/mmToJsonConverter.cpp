...
EntryHeatLaneTime
getHeatLaneTime(const string &eventRound, const string &stroke, float seedTime,
  ...
  // For diving events, the times are really the diving scores so leave those
  // alone
  if (stroke != "Diving") {
...
mmToJsonConverter::mmToJsonConverter(const string &mbFileSpec) {
  ...
  if (!dbConn_.connected()) {
    throw runtime_error("Error connecting to the database file.");
  }
  // cout << "mmToJsonConverter::ctor - connected to the db, driver = " <<
  // dbConn_.driver_name() << endl;
}
...
void mmToJsonConverter::addRelayEntries(Event &event) {
  ...
  while (result.next()) {
    ...
    EntryHeatLaneTime entryInfo = getHeatLaneTime(
        event.getRound(), event.getStroke(), seedTime, preHeat, preLane,
        preTime, preStat, finHeat, finLane, finTime, finStat);

    if (entryInfo.heat != 0 && entryInfo.lane != 0) {
      string teamName = getTeamNameByNumber(teamNo);
      vector<Athlete> athletes = getRelayAthletes(event.getEventPtr(), teamNo,
                                                  relayLtr, event.getRound());

      RelayEntry entry(relayLtr, athletes, entryInfo.heat, entryInfo.lane,
                       teamName, entryInfo.seed, entryInfo.time);
...
void mmToJsonConverter::addIndividualEntries(Event &event) {
  ...
  while (result.next()) {
    ...
    if (entryInfo.heat != 0 && entryInfo.lane != 0) {
      ...
      IndEntry entry(athlete, entryInfo.heat, entryInfo.lane, teamName,
                     entryInfo.seed, entryInfo.time);
      event.addEntry(entry);
    }
  }
}
...
map<int, Athlete> mmToJsonConverter::createAthleteMap() {
  ...
  string sql = "Select Ath_no, First_name, Last_name, Team_no, Schl_yr, "
               "Ath_age From Athlete";
...
string
mmToJsonConverter::createConnectionString(const string &mdbFileSpec) const {
  string dbConnString;
  ...
  dbConnString = string("Driver=") + MS_ACCESS_DRIVER + string(";") +
                 string("Dbq=") + mdbFileSpec + string(";") + string("Uid=") +
                 MM_USER_ID + string(";") + string("Pwd=") + MM_PASSWORD;
#else
  // On macOS/Linux, we typically use MDBTools driver
  // Just specifying the file is often enough for MDBTools if we don't use DSN
  // But nanodbc expects a connection string.
  // Driver={MDBTools};DBQ=file.mdb;
  const string DRIVER = "{MDBTools}";
  // Use the constants defined above
  dbConnString = string("Driver=") + DRIVER + string(";") + string("Dbq=") +
                 mdbFileSpec + string(";") + string("Uid=") + MM_USER_ID +
                 string(";") + string("Pwd=") + MM_PASSWORD;
#endif
...
vector<Event> mmToJsonConverter::getAllEvents() {
  vector<Event> events;
  string sql = "Select Event_no, Ind_rel, Event_gender, Event_sex, Low_age, "
               "High_age, Event_dist, "
  ...
  while (result.next()) {
    ...
    // Events from a "meet" rather than a session are always Final and the event
    // pointer is the same as the event number.
    Event event(number, relay, gender, genderDesc, minAge, maxAge, distance,
                stroke, division, "F", number, numLanes);
...
vector<EventRound> mmToJsonConverter::getEventRounds(int sessionPtr) {
  // BINDING DOES NOT WORK, probably due to MS Access driver.  Fails in the
  // statement.bind();
  ...
  string sql = "Select Sess_order, Sess_ptr, Sess_rnd, Event_ptr from Sessitem "
               "where Sess_ptr =" +
               sessPtrVal + " order by Sess_order";
  nanodbc::result result = nanodbc::execute(dbConn_, sql);
...
Event mmToJsonConverter::getEventById(EventRound eventRound) {
  ...
  string sql = "Select Event_no, Ind_rel, Event_gender, Event_sex, Low_age, "
               "High_age, Event_dist, "
               "Event_stroke, Div_no, Num_prelanes, Num_finlanes, Event_rounds "
               "From Event where Event_Ptr = " +
               eventIdStr;
  nanodbc::result result = nanodbc::execute(dbConn_, sql);

  if (!result.next()) {
    string errStr = "Error in mmToJsonConverter::getEventById, no event found "
                    "with event_ptr = " +
                    eventIdStr;
    ...
    // cout << "getEventById: pre=" << preLanes << ", fin=" << finLanes << ",
    // evtRounds=" << evtRounds << endl;
...
unique_ptr<Meet> mmToJsonConverter::getMeetInfo() {
  ...
  string sql =
      "select Meet_name1, Meet_location, Meet_start, Meet_end, Meet_class, "
...
vector<Athlete> mmToJsonConverter::getRelayAthletes(int eventId, int teamNo,
                                                    const string &teamLtr,
                                                    const string &round) {
  vector<Athlete> athletes;
  char sqlBuf[256] = {0};

  snprintf(sqlBuf, sizeof(sqlBuf),
           "Select Ath_no from RelayNames where Event_ptr = %d AND team_no = "
           "%d AND Team_ltr = '%s' AND Event_round = '%s'",
...
vector<Session> mmToJsonConverter::getSessionInfo() {
  ...
  while (result.next()) {
    ...
    Session session(sessionPtr, sessionNumber, sessionName, sessionDay,
                    sessionStart);
