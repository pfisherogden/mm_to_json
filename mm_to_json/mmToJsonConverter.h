...
public:
  // Create a converter for the Meet Manager database file identified by
  // mbFileSpec. Throws an xxx exception if the database file cannot be opened.
  ...
  static inline std::string toJsonStr(const nlohmann::json &json) {
    return json.dump(4);
  }
...
private:
  ...
  std::vector<Athlete> getRelayAthletes(int eventId, int teamNo,
                                        const std::string &teamLtr,
  ...
  std::map<int, std::string>
      divisionMap_; // cache of division IDs to division name
