...
class MmToJsonConverter:
    def __init__(self, mdb_path, password=None):
        ...
        for logical, physical_candidates in self.table_aliases.items():
            ...
            if found_name:
                ...
                if logical != 'Sessitem' or self.schema_type == 'A':
    ...
    def get_meet_info(self):
        df = self.tables['Meet']
        if df.empty:
            return {'meetName': '', 'meetLocation': '', 'meetStart': '', 'meetEnd': '', 'meetType': 0, 'numLanes': 0}
        ...
        row = df.iloc[0]
        def get(col, default): return row[col] if col in row else default
    ...
    def _parse_lo_hi(self, val):
        ...
        if val == 0: return 0, 109 # Open
        if val < 10: return 0, val # e.g. 8 -> 8&U
        s = str(val)
        if len(s) == 3: # 910
    ...
    def add_individual_entries(self, event):
        ...
        if self.schema_type == 'B':
            ...
            # Schema A
            if 'Event_ptr' not in df.columns: return
        ...
        if self.schema_type == 'B':
            ...
            if 'MtEvent' in df.columns:
                 ...
                 for _, row in entries.iterrows():
                     ...
                     time_str = "NT"
                     if score > 0:
                          if score > 200: time_str = self.num_to_string(score / 100.0)
                          else: time_str = self.num_to_string(score)
    ...
    def get_stroke(self, stroke_id, is_relay):
        if not stroke_id: return ""
        sid = str(stroke_id)[0]
        
        # Numeric checks for Schema B
        if str(stroke_id) == '1': return "Freestyle"
        if str(stroke_id) == '2': return "Backstroke"
        if str(stroke_id) == '3': return "Breaststroke"
        if str(stroke_id) == '4': return "Butterfly"
        if str(stroke_id) == '5': return "Individual Medley" if not is_relay else "Medley"
        
        if sid == 'A': return "Freestyle"
        if sid == 'B': return "Backstroke"
        if sid == 'C': return "Breaststroke"
        if sid == 'D': return "Butterfly"
        if sid == 'E': return "Medley" if is_relay else "Individual Medley"
        if sid == 'F': return "Diving"
        return ""
    ...
    def time_to_string(self, time_val, status):
        # logic from util.h
        if status and status.upper() == "SCR": return "SCR"
        if status and status.upper() == "DNS": return "DNS"
        if status and status.upper() == "DNF": return "DNF"
        if status and status.upper() == "DQ":  return "DQ"
        if time_val == 0.0: return "NT"
...
class Event:
    def __init__(self, event_no, is_relay, gender, gender_desc, min_age, max_age, distance, stroke, division, round_ltr, event_ptr, num_lanes):
    ...
    def create_description(self, meet_type):
        ...
        # age str
        age_str = ""
        if self.min_age == 0 and self.max_age == 109: age_str = "Open"
        elif self.min_age == 0: age_str = f"{self.max_age} & Under"
        elif self.max_age >= 109: age_str = f"{self.min_age} & Over"
        else: age_str = f"{self.min_age} - {self.max_age}"
...
def main():
    ...
    parser.add_argument("-p", "--password", help="Database password (optional). Can also set MM_DB_PASSWORD env var.")
